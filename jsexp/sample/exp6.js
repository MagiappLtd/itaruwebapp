...
var EXP = (function () {
    ...
    var BUTTON_WIDTH = "200px";
    var BUTTON_HEIGHT = "100px";
    var ID_BUTTON_L = "button_l";
    var BUTTON_L_LABEL = "母音";
    var ID_BUTTON_R = "button_r";
    var BUTTON_R_LABEL = "子音";
    ...
    var OPTION = { timeout: ["制限時間",
			     [{ name: "あり", value: TIMEOUT },
			      { name: "なし", value: NO_TIMEOUT } ]] };

    // ***** 実験開始に先立って1回だけ実施する処理
    // 実験全体に共通する教示・ブロック実施順の決定など
    // block_prologueを呼ぶこと
    var prologue = function () {
	IO.select_option("実験者の指示に従って条件を選んでください",
			 ID_MAIN, OPTION, prologue2);
    };
	...
	var set_screen = function () {
	    ...
	    IO.display_instruction("【サンプル実験】",
				   (OPTION.timeout.value === NO_TIMEOUT ?
				   ["画面に+が表示されたあとアルファベットが1つ表示されます。表示された文字が母音かどうかを判断し、母音だったら右手の人差指でJのキー、子音だったら左手の人差指でFのキーをできるだけ早く正確に押してください。"] :
				   ["画面に+が表示されたあとアルファベットが1つ表示されます。表示された文字が母音かどうかを判断し、母音だったら右手の人差指でJのキー、子音だったら左手の人差指でFのキーをできるだけ早く正確に押してください。この実験には制限時間があります。"]), 
				   ID_MAIN);
	};
	...
	if (block.type === PRACTICE) {
	    IO.append_paragraph("スペースキーを押すと練習を開始します。",
				ID_MAIN, { textAlign: "center" });
	} else if (block.type === EXPERIMENT) {
	    IO.append_paragraph("スペースキーを押すと" + b + "番目のブロックを開始します。",
				ID_MAIN, { textAlign: "center" });
	}
	if (OPTION.timeout.value !== NO_TIMEOUT) {
	    msg = "この実験には制限時間があり、" + (OPTION.timeout.value / 1000) +
	...
	var set_screen = function () {
	    ...
	    IO.append_element('img', ID_CENTER,
			      ...
			      ID_MAIN, "center");
	    IO.append_element('button', ID_BUTTON_L,
			      {width: BUTTON_WIDTH, height: BUTTON_HEIGHT},
			      ID_MAIN);
	    IO.append_element('button', ID_BUTTON_R,
			      {width: BUTTON_WIDTH, height: BUTTON_HEIGHT},
			      ID_MAIN);
	    IO.place_element(ID_BUTTON_L, 140, 280);
	    IO.place_element(ID_BUTTON_R, 540, 280);
	    IO.set_html(BUTTON_L_LABEL, ID_BUTTON_L);
	    IO.set_html(BUTTON_R_LABEL, ID_BUTTON_R);
	};
	...
	IO.present_stimuli_and_wait_for_key(
	    [[IMAGES["asterisk"], T_FIX, ID_CENTER],
	     ...
	     [IMAGES[trial.letter], OPTION.timeout.value, ID_CENTER]],
	    [KEY_YES, KEY_NO],
	    set_reaction);
    };
