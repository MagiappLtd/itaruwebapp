...
var EXP = (function () {
    // ***** 必須の変数・定数
    var TITLE = "muller-lyer_mixed";    // 結果データを保存するディレクトリ名に使われる
    ...
    var make_trials = function (src) {
        ...
        src.forEach(function (s) {
            trials.push( { angle: s[0], length: s[1], result: null } );
        });
        return trials;
    };
    ...
    var blocks = [
        { name: "本試行",
          trials: make_trials(
              [ [15, 30, 45, 60], [30, 60, 90] ].combine()
          )
        }
    ];

    var OPTION = { length: ["長さ",
                            [{ name: "短", value: 30 },
                             { name: "中", value: 60 },
                             { name: "長", value: 90 }]] };
    ...
    var prologue2 = function () {
        ...
        setup();
        IO.clear(ID_MAIN);
        IO.display_instruction("【Muller-Lyer錯視】",
                               [["左右の直線が同じ長さになるようボタンをクリックして調整し，その後終了ボタンを押してください。", "center"]],
                               ID_MAIN);
        block_prologue();
    };
    ...
    var block_prologue = function () {
        var setup = function () {
            ...
            block.trials.forEach(function (trial) {
                if (trial.length === OPTION.length.value) {
                    selected_trials.push(trial);
                }
            });
            block.trials = selected_trials.shuffle();
            t = 0;
        };
    ...
    var do_trial = function () {
        var set_screen = function () {
            ...
            IO.set_html("<p>左右の直線が同じ長さになるようボタンをクリックして調節し、" +
                        "その後終了ボタンを押してください。</p>" +
                        "<canvas id='cvs' width='670px' height='500px'></canvas>" +
                        "<form name='control' width='670px' height='50px'>" +
    ...
    var draw = function () {
        ...
        var xoffset = trial.length * Math.cos(trial.angle * Math.PI / 180);
        var yoffset = trial.length * Math.sin(trial.angle * Math.PI / 180);

        canvas.clearRect(0, 0, 670, 500);
        canvas.beginPath();
        canvas.lineWidth = 2;
        canvas.strokeStyle="#000000";
        canvas.moveTo(120, 200);
        canvas.lineTo(550, 200);
        canvas.moveTo(120, 200);
        canvas.lineTo(120 - xoffset, 200 - yoffset);
        canvas.moveTo(120, 200);
        canvas.lineTo(120 - xoffset, 200 + yoffset);
        canvas.moveTo(550, 200);
        canvas.lineTo(550 - xoffset, 200 - yoffset);
        canvas.moveTo(550, 200);
        canvas.lineTo(550 - xoffset, 200 + yoffset);

        canvas.moveTo(200 + length, 200);
        canvas.lineTo(200 + xoffset + length, 200 - yoffset);
        canvas.moveTo(200 + length, 200);
        canvas.lineTo(200 + xoffset + length, 200 + yoffset);
        canvas.stroke();
    };
    ...
    var next_trial = function () {
        ...
        trial.result = (DEBUG ? IO.make_random_number(-20, 20) : 135 - length);
        set_screen();
        advance();
    };
