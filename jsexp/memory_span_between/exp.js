...
var EXP = (function () {
    // ***** 必須の変数・定数
    var TITLE = "memory_span_between";  // 結果データを保存するディレクトリ名に使われる
    ...
    var prologue2 = function () {
        var setup = function () {
            var new_blocks = [];
            // 選択されたsoaのブロック対(練習+本試行)だけ実施する
            blocks.forEach(function (block) {
                if (block[0].soa === OPTION.soa.value) {
                    new_blocks.push(block);
                }
            });
            blocks = new_blocks.shuffle().flatten1();
            b = 0;
        };
