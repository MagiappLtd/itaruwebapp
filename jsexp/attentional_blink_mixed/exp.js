...
var EXP = (function () {
    // ***** 必須の変数・定数
    var TITLE = "attentional_blink_mixed";      // 結果データを保存するディレクトリ名に使われる
    ...
    var prologue2 = function () {
        var setup = function () {
            // 選択された条件のブロックだけ選んでblocksを設定しなおす
            var selected_blocks = [];
            blocks.forEach(function (block) {
                if (block[0].distracter_type === OPTION.distracter.value) {
                    selected_blocks.push(block);
                }
            });
            blocks = selected_blocks.shuffle().flatten1();
            b = 0;
        };
